...
import jieyi.tools.commuclient_8583.util.Des;

public class MacCalculator {
	private final static Logger logger = Logger.getLogger(MacCalculator.class);
	...
	// DesMac
	private byte[] MAC(int[][] cipherKey, byte[] bInit, byte[] bCiphertext) {
		byte pbySrcTemp[] = new byte[8];
		byte pbyInitData[] = new byte[8];
		byte pbyDeaSrc[] = new byte[8];
		byte pbyMac[] = new byte[4];
		...
		for (i = 0; i < n; i++) {
			...
			pbyInitData = encrypt(pbyDeaSrc, cipherKey);
		}
	...
	/*
	 *  DES 
	 *  strKey:密钥,Hex字符串,如:78B49F4BF5B16A17 .长度必须为16
	 *  strInitData:初始因子.长度必须为16,一般为:0000000000000000
	 *  strMacData:MAC数据,长度必须为偶数
	 */
	private String StrMAC(String strKey, String strInitData,
			String strMacData) {
		if ((strKey.length()) != 16) {
		...
		//byte[] cipherKey1 = hex2byte(strKey1.getBytes()); //3DES的密钥K1
		//byte[] cipherKey2 = hex2byte(strKey2.getBytes()); //3DES的密钥K2
		byte[] cipherKey = hex2byte(strKey.getBytes());
		byte[] bInit = hex2byte(strInitData.getBytes()); //初始因子
		byte[] bCiphertext = hex2byte(strMacData.getBytes()); //MAC数据
		
		int[][] subKeys = new int[16][48]; //用于存放产生的密钥
		subKeys = makeSubKeys(cipherKey);
		
		byte[] byMac = MAC(subKeys, bInit, bCiphertext);

		return byte2hex(byMac);
	}
	
	public String calculatorMac3Des(String communicationKey, String strData){
		System.out.println(strData);
		return this.Str3MAC(communicationKey, "0000000000000000", strData);
	}
	
	public String calculatorMacDes(String communicationKey, String strData){
		System.out.println(strData);
		return this.StrMAC(communicationKey, "0000000000000000", strData);
	}
	 ...
	 * @return
	 */
	public String macx99(String hexData, String hexKey){
		if ((hexKey.length()) != 16) {
			throw new IllegalArgumentException("密钥长度不正确,必须为16");
		}
		...
		byte[] byteEncLeft = Des.EncryptRetByte(sHexTmp.substring(0, 8).getBytes(), byteKey);
		...
		byte[] byteEncTmp = Des.EncryptRetByte(byteTmp, byteKey);
	...
	public String StrDe3DES(String strKey, String strEncData){
		...
		byte[] bTemp11 = decrypt(bCiphertext1, subKeys1);      //解
		byte[] bTemp21 = encrypt(bTemp11, subKeys2);           //加
		byte[] bPlaintext11 = decrypt(bTemp21, subKeys1);      //解
	...
	public String StrEn3DES(String strKey, String strEncData){
		String strKey1;
		String strKey2;
		String strTemp1;
		String strTemp2;
	...
	/*	if ((strEncData.length()) != 32) {
		...
		byte[] bTemp11 = encrypt(bCiphertext1, subKeys1);      //解
		byte[] bTemp21 = decrypt(bTemp11, subKeys2);           //加
		byte[] bPlaintext11 = encrypt(bTemp21, subKeys1);      //解
	...
	public List<String> getEncData(String strkey, String strEncdata){
		int length = strEncdata.length();
		if(length%32 != 0){
			strEncdata = StringUtils.rightPad(strEncdata, (strEncdata.length()/32 + 1) * 32, "0");
		}
		
		String strDecodeData = "";
		MacCalculator m =  new MacCalculator();
		for(int i = 0; i < strEncdata.length() /32; ++i){
			strDecodeData +=m.StrDe3DES(strkey, strEncdata.substring(i*32,i*32 + 32));
		}
	
		strDecodeData = strDecodeData.substring(0, length);
		
		List<String> data = new ArrayList<String>();
		getLVData(strDecodeData, data);
		
		return data;
	}
	
	public void getLVData(String str, List<String> data){
		if(str.equals("")){
			return ;
		}
		...
		if(length + 3 >= str.length()){
			data.add(str.substring(3));
			return;
		}
		
		data.add(str.substring(3, 3+length));
		getLVData(str.substring(length +3), data);
	}
	
	public static void main(String[] arg){
		MacCalculator m =  new MacCalculator();
		//String s1 = m.calculatorMac3Des("78B49F4BF5B16A17DF4AF5A36E49F4A0", "1234");
		//String s2 = m.calculatorMacDes("78B49F4BF5B16A17", "1234");
		//System.out.println(s1+"|"+s2);
		
		
		//String s3 = m.macx99("", 
		//		"E3C11C97C1089426");
		//System.out.println(s3);
	/*	String s1 = 
			"BE7987C865A604E80C765D0B678F2C5297EE0E307D4F105885EFB6D609993A0B5991C7AA38E65C9CA6A8980A64EA0198837D2C6DAD014D94C5BB54887DA92FDB06C67AF66C13FB05";
		String key = "12345678901234567890888888888888";
		/*for(int i = 0; i < s1.length()/32; ++i){
			String s4 = m.StrDe3DES(key, s1.substring(i*32,i*32 + 32));
			System.out.println("data:" + s1.substring(i*32,i*32 + 32) + "   " + "dataenc:" + s4);
		}*/
	/*	List<String> data = m.getEncData(key,s1);
		for(String str : data){
		...
		//String key = "4ad6e783bd8f6b0474ff1d85426cebbd"; // 22
		//String key = "1d4e326fc37ea3c8ce85b84c2dd181a9"; // 23
		
		//String key = "021ceb480753dd76f62b38dce3c36c03"; //24
		//String key = "ac1ea22f822ecacd8191c398689a171e"; //25
		//String key = "77764a7a1f74c4f4dc3a585302be9193"; //26
		//String key = "f04ba0fbccb82c40cc54f16c20662b22"; //27
		
		String key = "fe3488cec1cc6650e91e7397291fb29d"; 

		//String midata = "B253767D33220011F9DD307E638F180E";  //27 1101
		String midata = "61E4DE6A08BBA9DB7A1EFDB172C7C58C";
		
		String data = m.StrDe3DES(key, midata);
