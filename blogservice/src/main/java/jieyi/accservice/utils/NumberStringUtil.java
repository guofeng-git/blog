...
public class NumberStringUtil {
	...
	private static Logger logger = Logger.getLogger(NumberStringUtil.class);
	...
	*/  
	 ...
	 * @return
	 */
	public static String addLeftSpace(String str,int length){
		int str_length = str.length();
		for(int i=0; i<(length-str_length); i=i+1){
			str = ' ' + str;
		}
	...
	public static final String getCheckCode(String info,int len){
        byte x = 0x00;
        byte[] ss= new byte[1];
        for(int i=0;i<len;i++){
            String s = info.substring(i*4, i*4+4);
            byte y = (StringUtil.hexStringToBytes(s))[0];
            x = (byte) (x^y);
        }
        ss[0] = x;
        String calStr = StringUtil.bytesToHexString(ss).toUpperCase();
        return calStr;
    }
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		String name = "武大郎34";
		String result = NumberStringUtil.bytesToHexString(name.getBytes("utf-8"));
		System.out.println(NumberStringUtil.bytesToHexString(name.getBytes("utf-8")));
		String ll = NumberStringUtil.bytesToHexString(result.getBytes());
		System.out.println(ll);
		String num = "20"; 
		System.out.println( NumberStringUtil.bytesToHexString(num.getBytes()));
	}
}
